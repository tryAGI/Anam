...
internal static partial class PersonasCreatePersonaCommandApiCommand
{
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                        var __requestBase = await CliRuntime.ReadRequestOrDefaultAsync<global::Anam.PersonaConfig>(
                            ...
                            cancellationToken).ConfigureAwait(false);                        var id = CliRuntime.WasSpecified(parseResult, PersonaConfigOptionSetOptions.Id) ? parseResult.GetValue(PersonaConfigOptionSetOptions.Id) : (__requestBase is { } __IdBaseValue ? __IdBaseValue.Id : default);
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.Personas.CreatePersonaAsync(
                                    id: id,
                                    name: name,
